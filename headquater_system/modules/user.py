...
# 사용자 정보를 담는 클래스 정의
class User:
    def __init__(self, name: str, source_lang: str = "ko", target_lang: str = "en"):
        self.name = name
...
def get_or_create_user(name: str, default_source: str = "ko", default_target: str = "en", websocket=None) -> User:
    with users_lock:
        if name in users:
            user = users[name]
            ...
            user = User(name, default_source, default_target)
            user.websocket = websocket
            users[name] = user
            print(f"[DEBUG] 새로운 사용자가 추가되었습니다: {user.name}")
            return user
